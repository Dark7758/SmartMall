<template>
  <div class="pay">
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <!-- 买家留言 -->
      <div class="buytips">
        <textarea placeholder="选填：买家留言（50字内）" v-model="remark" name="" id="" cols="30" rows="10"></textarea>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async goPay () {
      ...
      if (this.mode === 'buyNow') {
        const { data: { order, personal } } = await checkOrder(this.mode, { goodsId: this.goodsId, goodsSkuId: this.goodsSkuId, goodsNum: this.goodsNum })
        this.order = order
        this.personal = personal
      }
    },
    ...
    // async addAddress (name, phone, region) {
  ...
  computed: {
    ...
    longAddress () {
      const { region: { province, city, region }, detail } = this.defaultAddress
  ...
  async created () {
    // const name = '林冬'
    // const phone = '17627167253'
    // const region = [{ value: 782, label: '浙江省' }, { value: 783, label: '杭州市' }, { value: 785, label: '江干区' }]
    // const detail = '北京路1号楼8888室'
    // const res = await addAddress(name, phone, region, detail)
    // console.log(res)
